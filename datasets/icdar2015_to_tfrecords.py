...
def cvt_to_tfrecords(output_path , data_path, gt_path):
    ...
    with tf.python_io.TFRecordWriter(output_path) as tfrecord_writer:
        for idx, image_name in enumerate(image_names):
            ...
            labels_text = [x.encode() for x in labels_text] # str -> bytes 
            image_name = [x.encode() for x in image_name] # str -> bytes 
...
if __name__ == "__main__":
    # train set
    output_path = "tfrecords_train"
    data_path = "icdar2015/ch4_training_images"
    gt_path = "icdar2015/ch4_training_localization_transcription_gt"
    cvt_to_tfrecords(output_path , data_path, gt_path)
    #  test set 
    output_path = "tfrecords_test"
    data_path = "icdar2015/ch4_test_images"
    gt_path = "icdar2015/ch4_test_localization_transcription_gt"
    cvt_to_tfrecords(output_path , data_path, gt_path)
